...
|----------|--------------|-------|
| Soil sensor analog OUT | GPIO26 (<GlossaryTerm term="ADC" />) | Higher reading = drier soil |
| Valve PWM (MOSFET gate) | GPIO0 | Logic-level <GlossaryTerm term="PWM" /> (3.3 V) |
| RGB LED data | GPIO25 | On-board <GlossaryTerm term="LED" />LED|
...
- Valve or pump **positive** → 5-12 V supply (directly or from a <GlossaryTerm term="DC" />-<GlossaryTerm term="DC" /> step-up module)
- Valve or pump **negative** → <GlossaryTerm term="MOSFET" /> drain
- <GlossaryTerm term="MOSFET" /> source → GND
- <GlossaryTerm term="MOSFET" /> gate → GPIO0 (optional 100-220 Ω series resistor in the gate line)
